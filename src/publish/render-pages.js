...
function renderSharedStyles() {
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    /* ---------- At-a-glance summary ---------- */
    .glance-headline { font-size: 1.15rem; margin: 0.5rem 0 1rem; }
    .glance-facts { margin: 0.75rem 0 0.25rem; padding-left: 1.25rem; }
    .glance-facts li { margin-bottom: 0.4rem; }
    ...
    .audience-card h3 { margin: 0 0 0.5rem; font-size: 1rem; }
    .audience-card ul { margin: 0.5rem 0 0; padding-left: 1.25rem; }
    ...
    .page-toc a:hover { background: var(--color-badge-hover); text-decoration: underline; }
    .page-toc a:focus-visible { outline: 3px solid var(--color-focus-ring); outline-offset: 2px; }
...
function computeTotalExcluded(report) {
  ...
  return roundDownConservatively(
    Object.values(categories).reduce((sum, data) => sum + (data.estimated_excluded_users ?? 0), 0)
  );
}

function renderAtAGlanceSection(report) {
  const counts = report.url_counts;
  const scores = report.aggregate_scores;
  const topUrls = report.top_urls ?? [];

  const totalExcluded = computeTotalExcluded(report);
  ...
  if (topPattern) {
    const pageWord = topPattern.count === 1 ? 'page' : 'pages';
    const title = String(topPattern.title ?? '').replace(/\.\s*$/, '');
    facts.push(`<li>Most widespread issue: <strong>${escapeHtml(title)}</strong>, found on ${topPattern.count} ${pageWord}. <a href="#axe-patterns-heading">See the most common issues</a>.</li>`);
  }
...
function renderHowToUseSection() {
  ...
  <section aria-labelledby="how-to-use-heading">
    <h2 id="how-to-use-heading">How to Use This Report${renderAnchorLink('how-to-use-heading', 'How to Use This Report')}</h2>
    <p>Different readers need different things from this data. Start where it is most relevant to you.</p>
    <div class="audience-grid">
      <div class="audience-card">
        <h3>Work on a government website?</h3>
        <ul>
          <li>Find your site in the <a href="#top-urls-heading">Top URLs table</a> and compare its scores with peer agencies.</li>
          <li>Open <strong>Details</strong> for your page: every finding has a stable ID and a copy button, ready to paste into a ticket.</li>
          <li>Findings keep the same ID from day to day, so you can confirm a fix actually shipped.</li>
        </ul>
      </div>
      <div class="audience-card">
        <h3>Reporting or researching?</h3>
        <ul>
          <li>Read the <a href="press-release.md">plain-language daily summary</a> of the biggest barriers and who they affect.</li>
          <li>Cite the <a href="#narrative-heading">trend narrative</a> and the <a href="#day-comparison-heading">comparison with the recent average</a>.</li>
          <li>Download the raw data (CSV and JSON links throughout) and check the <a href="#dap-context-heading">methodology</a>.</li>
        </ul>
      </div>
      <div class="audience-card">
        <h3>Want things to improve?</h3>
        <ul>
          <li>Share the <a href="#fpc-exclusion-heading">estimate of Americans excluded</a> — it makes the stakes concrete.</li>
          <li>See <a href="#cta-heading">Take Action</a> for concrete steps: free testing tools, USWDS adoption, and hiring people with disabilities.</li>
        </ul>
      </div>
    </div>
  </section>`;
...
function renderOnThisPageNav(sections) {
  const links = sections
    .map((s) => `<li><a href="#${escapeHtml(s.id)}">${escapeHtml(s.label)}</a></li>`)
    .join('\n      ');
  return `
  <nav class="page-toc print-hide" aria-labelledby="toc-heading">
...
function renderHistorySection(report) {
  return `
...
function renderTopUrlsSection(report) {
  return `
...
export function renderDailyReportPage(report) {
  const detailSections = [
    { id: 'narrative-heading', label: 'Trend Narrative', html: renderNarrativeSection(report) },
    { id: 'day-comparison-heading', label: 'Comparison with Average', html: renderDayComparisonSection(report) },
    { id: 'fpc-exclusion-heading', label: 'Americans Excluded', html: renderFpcExclusionSection(report) },
    { id: 'axe-patterns-heading', label: 'Common Issues', html: renderAxePatternsSection(report.top_urls) },
    { id: 'usability-heuristics-heading', label: 'Usability Heuristics', html: renderUsabilityHeuristicsSection(report.top_urls) },
    { id: 'performance-impact-heading', label: 'Performance Impact', html: renderPerformanceImpactSection(report) },
    { id: 'content-density-heading', label: 'Content Density', html: renderContentDensitySection(report) },
    { id: 'environmental-conditions-heading', label: 'Environmental Conditions', html: renderEnvironmentalConditionsSection(report) },
    { id: 'history-heading', label: 'History', html: renderHistorySection(report) },
    { id: 'top-urls-heading', label: 'Top URLs', html: renderTopUrlsSection(report) },
    { id: 'tech-summary-heading', label: 'Technologies', html: renderTechSummarySection(report) },
    { id: 'compliance-context-heading', label: 'Section 508 Context', html: renderComplianceContextSection() },
    { id: 'dap-context-heading', label: 'About These Reports', html: renderDapContextSection() },
    { id: 'cta-heading', label: 'Take Action', html: renderCallToActionSection(report) }
...
<body>
  ...
  <main id="main-content" class="site-main">
    <div class="page-intro">
      ...
      <p><a href="code-quality.html">View HTML/CSS/JS Code Quality report for this date &rarr;</a> &middot; New here? <a href="#dap-context-heading">Learn how these reports work</a>.</p>
    </div>

    ${renderAtAGlanceSection(report)}

    ${renderHowToUseSection()}

    ${renderOnThisPageNav(detailSections)}
