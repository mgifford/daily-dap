 ...
 *  - CMS platform: WordPress, Drupal, Joomla, ASP.NET, SharePoint, Squarespace,
 *    Adobe Experience Manager, and TYPO3
...
const CMS_PATTERNS = {
  ...
  // ASP.NET is widely used by U.S. federal and state government websites.
  // Detected from characteristic URL patterns generated by the framework.
  'ASP.NET': [
    /\/(WebResource|ScriptResource)\.axd(\?|$)/i,
    /\/aspnet_client\//i,
    /__doPostBack/i,
    /aspnetcdn\.com/i
  ],
  // Microsoft SharePoint is common in government intranets and some public sites.
  SharePoint: [
    /\/_layouts\/15\//i,
    /\/_vti_bin\//i,
    /\/_catalogs\/(masterpage|theme|lt)\//i,
    /\/_api\/web\//i
  ],
  // Squarespace is occasionally used by government-adjacent and small agency sites.
  Squarespace: [
    /static\.squarespace\.com/i,
    /\/squarespace\//i,
    /assets\.squarespace\.com/i
  ],
  // Adobe Experience Manager (AEM) is used by several large federal agencies.
  'Adobe Experience Manager': [
    /\/etc\.clientlibs\//i,
    /\/content\/dam\//i,
    /\/libs\/granite\//i
  ],
  // TYPO3 is used on some government and academic sites.
  TYPO3: [
    /\/typo3(conf|temp)?\//i,
    /\/typo3\/sysext\//i,
    /EXT:frontend/i
 ...
 *   cms_avg_scores: Record<string, {performance: number|null, accessibility: number|null, best_practices: number|null, seo: number|null}>,
 *   uswds_count: number,
...
export function buildTechSummary(urlResults = []) {
  ...
  for (const result of successful) {
    ...
    if (tech.cms) {
      ...
      if (!cmsScoreAccum[tech.cms]) {
        cmsScoreAccum[tech.cms] = { performance: [], accessibility: [], best_practices: [], seo: [] };
      }
      const acc = cmsScoreAccum[tech.cms];
      if (typeof result.lighthouse_performance === 'number') acc.performance.push(result.lighthouse_performance);
      if (typeof result.lighthouse_accessibility === 'number') acc.accessibility.push(result.lighthouse_accessibility);
      if (typeof result.lighthouse_best_practices === 'number') acc.best_practices.push(result.lighthouse_best_practices);
      if (typeof result.lighthouse_seo === 'number') acc.seo.push(result.lighthouse_seo);
    }
  ...
  const cmsAvgScores = {};
  for (const [cms, acc] of Object.entries(cmsScoreAccum)) {
    const avg = (arr) => arr.length > 0 ? Math.round(arr.reduce((s, v) => s + v, 0) / arr.length) : null;
    cmsAvgScores[cms] = {
      performance: avg(acc.performance),
      accessibility: avg(acc.accessibility),
      best_practices: avg(acc.best_practices),
      seo: avg(acc.seo)
    };
  }
